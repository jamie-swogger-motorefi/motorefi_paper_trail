...
require "generators/motorefi_paper_trail/install/install_generator"

RSpec.describe MotorefiPaperTrail::InstallGenerator, type: :generator do
  ...
  describe "no options" do
    ...
    it "generates a migration for creating the 'motorefi_versions' table" do
          ...
          directory("db") {
            directory("migrate") {
              migration("create_versions") {
                ...
                contains "def change"
                contains "create_table :motorefi_versions#{expected_create_table_options}"
  ...
  describe "`--with-changes` option set to `true`" do
          ...
          directory("db") {
            directory("migrate") {
              migration("create_versions") {
                ...
                contains "def change"
                contains "create_table :motorefi_versions"
              }
            }
          }
        }
    ...
    it "generates a migration for adding the 'object_changes' column to the 'motorefi_versions' table" do
      expect(destination_root).to(
        have_structure {
          directory("db") {
            directory("migrate") {
              migration("add_object_changes_to_versions") {
                ...
                contains "def change"
                contains "add_column :motorefi_versions, :object_changes, :text"
              }
            }
          }
        }
  ...
  describe "`--uuid` option set to `true`" do
    ...
    it "generates a migration for creating the 'motorefi_versions' table with item_id type uuid" do
