...
module MotorefiPaperTrail
  ::RSpec.describe(Version, versioning: true) do
    ...
    describe ".not_creates" do
      it "returns all motorefi_versions except create events" do
        animal = Animal.create
        animal.update(name: "Animal")
        animal.destroy
        expect(
    ...
    describe ".subsequent" do
      context "with a timestamp" do
        it "returns all motorefi_versions that were created after the timestamp" do
          animal = Animal.create
          2.times do
            animal.update(name: FFaker::Lorem.word)
          end
          value = described_class.subsequent(1.hour.ago, true)
          expect(value).to eq(animal.motorefi_versions.to_a)
      ...
      context "with a Version" do
        it "grab the timestamp from the version and use that as the value" do
          ...
          expect(described_class.subsequent(animal.motorefi_versions.first)).to eq(
            animal.motorefi_versions.to_a.drop(1)
          )
        end
      end
    end
    ...
    describe ".preceding" do
      context "with a timestamp" do
        it "returns all motorefi_versions that were created before the timestamp" do
          animal = Animal.create
          2.times do
            animal.update(name: FFaker::Lorem.word)
          end
          value = described_class.preceding(5.seconds.from_now, true)
          expect(value).to eq(animal.motorefi_versions.reverse)
          expect(value.to_sql).to match(
      ...
      context "with a Version" do
        it "grab the timestamp from the version and use that as the value" do
          ...
          expect(described_class.preceding(animal.motorefi_versions.last)).to eq(
            animal.motorefi_versions.to_a.tap(&:pop).reverse
          )
        end
      end
    end
