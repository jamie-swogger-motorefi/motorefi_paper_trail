...
module MotorefiPaperTrail
  module Events
    ...
    #
    # The value inserted into the `event` column of the motorefi_versions table can also
    # be overridden by the user, with `motorefi_paper_trail_event`.
    #
    # @api private
    class Base
      ...
      def assert_metadatum_key_is_permitted(key)
        ...
        raise MotorefiPaperTrail::InvalidOption,
          format(E_FORBIDDEN_METADATA_KEY, key, FORBIDDEN_METADATA_KEYS)
      end

      # Rails 5.1 changed the API of `ActiveRecord::Dirty`. See
      # https://github.com/motorefi-paper-trail-gem/motorefi_paper_trail/pull/899
      #
      # @api private
      ...
      def nonskipped_attributes_before_change(is_touch)
        record_attributes = @record.attributes.except(*@record.motorefi_paper_trail_options[:skip])
      ...
      def calculated_ignored_array
        ignore = @record.motorefi_paper_trail_options[:ignore].dup
      ...
      def changed_and_not_ignored
        skip = @record.motorefi_paper_trail_options[:skip]
      ...
      # @api private
      def evaluate_only
        only = @record.motorefi_paper_trail_options[:only].dup
        # Remove Hash arguments and then evaluate whether the attributes (the
        # keys of the hash) should also get pushed into the collection.
      ...
      def ignored_attr_has_changed?
        ignored = calculated_ignored_array + @record.motorefi_paper_trail_options[:skip]
      ...
      def merge_item_subtype_into(data)
        if @record.class.motorefi_paper_trail.version_class.columns_hash.key?("item_subtype")
      ...
      def merge_metadata_from_controller_into(data)
        metadata = MotorefiPaperTrail.request.controller_info || {}
      ...
      def merge_metadata_from_model_into(data)
        @record.motorefi_paper_trail_options[:meta].each do |k, v|
      ...
      #
      # @api private
      def object_attrs_for_motorefi_paper_trail(is_touch)
      ...
      # serialization here, using `MotorefiPaperTrail.serializer`.
      #
      # @api private
      # @param changes HashWithIndifferentAccess
      def recordable_object_changes(changes)
        if MotorefiPaperTrail.config.object_changes_adapter.respond_to?(:diff)
          ...
          changes = MotorefiPaperTrail.config.object_changes_adapter.diff(changes.to_hash)
        end

        if @record.class.motorefi_paper_trail.version_class.object_changes_col_is_json?
          changes
        else
          MotorefiPaperTrail.serializer.dump(changes)
        end
      end
      ...
      def record_object_changes?
        @record.class.motorefi_paper_trail.version_class.column_names.include?("object_changes")
      end
      ...
      # @api private
      def record_object?
        @record.class.motorefi_paper_trail.version_class.column_names.include?("object")
      end
      ...
      # `MotorefiPaperTrail.serializer`.
      #
      # @api private
      def recordable_object(is_touch)
        if @record.class.motorefi_paper_trail.version_class.object_col_is_json?
          object_attrs_for_motorefi_paper_trail(is_touch)
        else
          MotorefiPaperTrail.serializer.dump(object_attrs_for_motorefi_paper_trail(is_touch))
        end
      end
