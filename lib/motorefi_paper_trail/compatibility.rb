...
module MotorefiPaperTrail
  # Rails does not follow SemVer, makes breaking changes in minor motorefi_versions.
  ...
  # [help of experts](https://github.com/motorefi-paper-trail-gem/motorefi_paper_trail/pull/899).
  #
  # It is not safe to assume that a new version of rails will be compatible with
  # MotorefiPaperTrail. PT is only compatible with the motorefi_versions of rails that it is
  # tested against. See `.github/workflows/test.yml`.
  #
  # However, as of
  # [#1213](https://github.com/motorefi-paper-trail-gem/motorefi_paper_trail/pull/1213) our
  # gemspec allows installation with newer, incompatible rails motorefi_versions. We hope
  # this will make it easier for contributors to work on compatibility with
  # newer rails motorefi_versions. Most PT users should avoid incompatible rails
  # motorefi_versions.
  module Compatibility
    ...
    E_INCOMPATIBLE_AR = <<-EOS
      MotorefiPaperTrail %s is not compatible with ActiveRecord %s. We allow PT
      contributors to install incompatible motorefi_versions of ActiveRecord, and this
      ...
      instead (%s). Please see the discussion in motorefi_paper_trail/compatibility.rb
      for details.
    EOS
    ...
    def self.check_activerecord(ar_version)
      ...
      unless req.satisfied_by?(ar_version)
        ::Kernel.warn(
          format(
            E_INCOMPATIBLE_AR,
            ::MotorefiPaperTrail.gem_version,
            ar_version,
            req
          )
        )
