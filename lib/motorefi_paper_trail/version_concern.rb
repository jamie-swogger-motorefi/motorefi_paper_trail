...
require "motorefi_paper_trail/queries/motorefi_versions/where_attribute_changes"
require "motorefi_paper_trail/queries/motorefi_versions/where_object"
require "motorefi_paper_trail/queries/motorefi_versions/where_object_changes"
require "motorefi_paper_trail/queries/motorefi_versions/where_object_changes_from"
require "motorefi_paper_trail/queries/motorefi_versions/where_object_changes_to"

module MotorefiPaperTrail
  # Originally, MotorefiPaperTrail did not provide this module, and all of this
  # functionality was in `MotorefiPaperTrail::Version`. That model still exists (and is
  ...
  module VersionConcern
    ...
    E_YAML_PERMITTED_CLASSES = <<-EOS.squish.freeze
      MotorefiPaperTrail encountered a Psych::DisallowedClass error during
    ...
    module ClassMethods
      ...
      # Given an attribute like `"name"`, query the `motorefi_versions.object_changes`
      ...
      # `motorefi_versions.objects` column.
      #
      # ```
      # SELECT "motorefi_versions".*
      # FROM "motorefi_versions"
      # WHERE ("motorefi_versions"."object" LIKE '%
      # name: Joan
      # %')
      # ```
      #
      # This is useful for finding motorefi_versions where a given attribute had a given
      # value. Imagine, in the example above, that Joan had changed her name
      # and we wanted to find the motorefi_versions before that change.
      #
      ...
      # Given a hash of attributes like `name: 'Joan'`, query the
      # `motorefi_versions.objects_changes` column.
      #
      # ```
      # SELECT "motorefi_versions".*
      # FROM "motorefi_versions"
      # WHERE .. ("motorefi_versions"."object_changes" LIKE '%
      # name:
      # - Joan
      # %' OR "motorefi_versions"."object_changes" LIKE '%
      # name:
      # -%
      # - Joan
      # %')
      # ```
      #
      # This is useful for finding motorefi_versions immediately before and after a given
      # attribute had a given value. Imagine, in the example above, that someone
      # changed their name to Joan and we wanted to find the motorefi_versions
      ...
      # Given a hash of attributes like `name: 'Joan'`, query the
      # `motorefi_versions.objects_changes` column for changes where the version changed
      # from the hash of attributes to other values.
      #
      # This is useful for finding motorefi_versions where the attribute started with a
      ...
      #
      # This is useful for finding motorefi_versions where the attribute started with an
      ...
      # Returns motorefi_versions before `obj`.
      #
      ...
      # Returns motorefi_versions after `obj`.
      #
      # @param obj - a `Version` or a timestamp
      # @param timestamp_arg - boolean - When true, `obj` is a timestamp.
      #   Default: false.
    ...
    def object_deserialized
      if self.class.object_col_is_json?
        ...
        MotorefiPaperTrail.serializer.load(object)
      end
    end
    ...
    #     comparing two motorefi_versions of the same object.
    ...
    def reify(options = {})
      ...
      ::MotorefiPaperTrail::Reifier.reify(self, options)
    end

    # Returns what changed in this version of the item.
    # `ActiveModel::Dirty#changes`. returns `nil` if your `motorefi_versions` table does
    ...
    def motorefi_paper_trail_originator
      @motorefi_paper_trail_originator ||= previous.try(:whodunnit)
    end
    ...
    # @api private
    def load_changeset
      if MotorefiPaperTrail.config.object_changes_adapter.respond_to?(:load_changeset)
        return MotorefiPaperTrail.config.object_changes_adapter.load_changeset(self)
      end
    ...
    def object_changes_deserialized
      if self.class.object_changes_col_is_json?
        ...
        begin
          MotorefiPaperTrail.serializer.load(object_changes)
    ...
    def version_limit
      ...
      if limit_option?(klass)
        klass.motorefi_paper_trail_options[:limit]
      elsif base_class_limit_option?(klass)
        klass.base_class.motorefi_paper_trail_options[:limit]
      else
        MotorefiPaperTrail.config.version_limit
      end
    end

    def limit_option?(klass)
      klass.respond_to?(:motorefi_paper_trail_options) && klass.motorefi_paper_trail_options.key?(:limit)
    end
