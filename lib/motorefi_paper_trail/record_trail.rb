...
require "motorefi_paper_trail/events/create"
require "motorefi_paper_trail/events/destroy"
require "motorefi_paper_trail/events/update"
...
module MotorefiPaperTrail
  ...
  class RecordTrail
    ...
    # Invoked after rollbacks to ensure motorefi_versions records are not created for
    ...
    # @api public
    def originator
      (source_version || motorefi_versions.last).try(:whodunnit)
    end
    ...
    def previous_version
      (source_version ? source_version.previous : motorefi_versions.last).try(:reify)
    end
    ...
    def record_create
      ...
      build_version_on_create(in_after_callback: true).tap do |version|
        ...
        # invalidate the `motorefi_versions` association cache with `reset`.
        motorefi_versions.reset
      end
    end
    ...
    # motorefi_paper_trail-association_tracking
    def record_destroy(recording_order)
      ...
      version = @record.class.motorefi_paper_trail.version_class.create(data)
    ...
    # @api private
    def save_version?
      if_condition = @record.motorefi_paper_trail_options[:if]
      unless_condition = @record.motorefi_paper_trail_options[:unless]
    ...
    def version_at(timestamp, reify_options = {})
      ...
      v = motorefi_versions.subsequent(timestamp, true).first
    ...
    def versions_between(start_time, end_time)
      motorefi_versions = send(@record.class.versions_association_name).between(start_time, end_time)
      motorefi_versions.collect { |version| version_at(version.created_at) }
    end
    ...
    def build_version_on_create(in_after_callback:)
      ...
      @record.class.motorefi_paper_trail.version_class.new(data)
    end
    ...
    def build_version_on_update(force:, in_after_callback:, is_touch:)
      ...
      # the association cache (see `motorefi_versions.reset`) and that could cause an
    ...
    def enabled?
      MotorefiPaperTrail.enabled? &&
        MotorefiPaperTrail.request.enabled? &&
        MotorefiPaperTrail.request.enabled_for_model?(@record.class)
    end
