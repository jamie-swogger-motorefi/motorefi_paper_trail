...
module MotorefiPaperTrail
  ...
  #
  # Please do not use `MotorefiPaperTrail::Request` directly, use `MotorefiPaperTrail.request`.
  ...
  module Request
    class << self
      # Sets any data from the controller that you want MotorefiPaperTrail to store.
      # See also `MotorefiPaperTrail::Rails::Controller#info_for_motorefi_paper_trail`.
      #
      #   MotorefiPaperTrail.request.controller_info = { ip: request_user_ip }
      #   MotorefiPaperTrail.request.controller_info # => { ip: '127.0.0.1' }
      #
      # @api public
      ...
      # Returns the data from the controller that you want MotorefiPaperTrail to store.
      # See also `MotorefiPaperTrail::Rails::Controller#info_for_motorefi_paper_trail`.
      #
      #   MotorefiPaperTrail.request.controller_info = { ip: request_user_ip }
      #   MotorefiPaperTrail.request.controller_info # => { ip: '127.0.0.1' }
      ...
      # Switches MotorefiPaperTrail off for the given model.
      # @api public
      def disable_model(model_class)
        enabled_for_model(model_class, false)
      end

      # Switches MotorefiPaperTrail on for the given model.
      # @api public
      def enable_model(model_class)
        enabled_for_model(model_class, true)
      end

      # Sets whether MotorefiPaperTrail is enabled or disabled for the current request.
      # @api public
      def enabled=(value)
        store[:enabled] = value
      end

      # Returns `true` if MotorefiPaperTrail is enabled for the request, `false` otherwise.
      # See `MotorefiPaperTrail::Rails::Controller#motorefi_paper_trail_enabled_for_controller`.
      # @api public
      def enabled?
        !!store[:enabled]
      end

      # Sets whether MotorefiPaperTrail is enabled or disabled for this model in the
      # current request.
      # @api public
      ...
      # Returns `true` if MotorefiPaperTrail is enabled for this model in the current
      # request, `false` otherwise.
      # @api public
      def enabled_for_model?(model)
        model.include?(::MotorefiPaperTrail::Model::InstanceMethods) &&
      ...
      # @api private
      def store
        RequestStore.store[:motorefi_paper_trail] ||= {
          enabled: true
        }
      end
